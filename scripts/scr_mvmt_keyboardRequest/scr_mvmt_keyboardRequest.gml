...
*/
var requestPriority = global.keyboardPriority
...
if (requestHSpeed != 0 || requestVSpeed != 0) {
	...
	show_debug_message(string(radians))
	var requestX = requestHSpeed * cos(radians) 
	var requestY = requestVSpeed * sin(radians)

	var requestDirection = radtodeg(radians)
		show_debug_message(string(requestDirection))
	var requestSpeed = sqrt( sqr(requestX) + sqr(requestY) )
show_debug_message(string(requestSpeed))

	scr_mvmt_moveRequest( id, requestDirection, requestSpeed , requestPriority)
	wantsToMove = true
